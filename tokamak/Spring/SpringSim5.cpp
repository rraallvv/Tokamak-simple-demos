...
void CSpringSim5::Init()
{
	...
	for (int i=0;i<4;i++)
	{
		CTokSpring * spring = this->CreateSpring(m_Rigid[m_RigidCount-1],m_Rigid[m_RigidCount-2]);
		spring->SetDefaultLength(25.0f);

		spring->SetSpringConst(0.2f);
		if (i<2)
			pos[0] = -8.0f;
		else 
			pos[0] = 8.0f;
		...
		fixInfo.fTorqueFactor = 200.0f;
		fixInfo.fAngularDamping = 5.0f;
		spring->FixSpring(fixInfo);
	}

}

void CSpringSim5::KeyDown(unsigned char key)
{
 if (key == 'p')
	{
